...
def download_aria2(url,id,mod):
    try:
        #print(os.path.join(os.getcwd(),name,id))
    ...
    for i in url:
        f.write(i+'\n')
    f.close()
    #print('aria2c --conf-path=./config/aria2.conf -d '+os.path.join(os.getcwd(),name,id)+' -i '+os.path.join(os.getcwd(),name,id,'file.txt'))
