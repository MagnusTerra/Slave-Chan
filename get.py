...
@retry(stop_max_attempt_number=5)
def get_bzdetail(id,dir_name):
    url_add = '/g/'+id+'/'
    try:
        os.mkdir(os.path.join(os.getcwd(),'bz',dir_name))
    except Exception:
        pass
    #r = requests.get(url_base+url_add,headers=headers,proxies=proxies,verify=False)
    r = requests.get(url_base+url_add,headers=headers)
    ...
    f = open(os.path.join(os.getcwd(),'bz',dir_name,'info.json'), mode='w+',encoding="utf-8")
    f.write(json.dumps(data,ensure_ascii=False))
